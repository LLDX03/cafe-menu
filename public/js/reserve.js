...
document.getElementById('resDate').addEventListener('change', (e) => {
    ...
    disablePastSlots(e.target.value);
...
function disablePastSlots(selectedDate) {
    ...
    // Only disable if selecting today
    if (selected.toDateString() === now.toDateString()) {
        const currentHour = now.getHours();

        document.querySelectorAll('.slot-btn:not(.full)').forEach(btn => {
            const time = btn.textContent.trim();
            const hour = parseInt(time);
            const isPM = time.includes('PM') && hour !== 12;
            const adjustedHour = isPM ? hour + 12 : (hour === 12 ? 0 : hour);
